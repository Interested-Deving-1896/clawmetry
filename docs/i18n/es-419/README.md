...
vista, para que puedas distinguir una ejecución larga que va bien de una que está atascada.
...
| **Qué sale de tu máquina** | Ningún dato de sesión, a menos que ejecutes `clawmetry connect`. Por defecto sí se ejecutan dos cosas, ambas opcionales y sin llevar contenido de sesión: un ping anónimo de instalación y una verificación de versión en PyPI. Cada destino está inventariado en [docs/EGRESS.md](docs/EGRESS.md), reconstruido a partir de una captura de tráfico en lugar de a partir de comentarios en el código. |

Hay dos límites que conviene conocer antes de juzgar el resultado: los runtimes exponen datos
...
reenfoca cada pestaña hacia uno de ellos.

¿Construiste tu propio agente sobre un SDK en vez de usar un runtime? El interceptor también rastrea
sus llamadas al LLM. Consulta [docs/SDK_TRACKING.md](docs/SDK_TRACKING.md).
...
- **Desbordamiento de contexto**: utilización de la ventana ajustada por proveedor, compactación frente a desbordamiento forzado, más un mapa por runtime de lo que *no* podemos ver ([cómo](docs/CONTEXT_BLOWOUT.md))
...
Un porcentaje de utilización es tan honesto como el denominador que usa. ClawMetry
...
realmente desbordado como un cómodo 65%.
...
medidor construido sobre una suposición nunca se muestra con la misma autoridad que uno construido
...
| Proxy de enforcement | **+9.7 ms** por llamada al LLM | desactivado |
...
mucha actividad. Ese último número supera nuestro propio presupuesto declarado de 5-10%, así que
se publica como un bug a perseguir en lugar de omitirlo de la página.
...
Cada ruta está medida, incluyendo los gates de hooks y el proxy de enforcement,
y el arnés corre en Linux, macOS y Windows en CI. Dos resultados que vale la pena
...
del 5-10%. El JSON crudo, el método, y lo que aún no está medido están en
...
Por defecto sí se ejecutan dos cosas antes de conectarte, ambas opcionales y sin
llevar datos de sesión: un ping anónimo de instalación y una verificación de versión contra
...
[docs/EGRESS.md](docs/EGRESS.md); las instalaciones autohospedadas, redirigidas y aisladas de la red
...
que se incluye dentro del wheel y se sirve tal cual, fijado con un hash de Subresource
...
que cualquiera puede descargar. Autohospedar o quedarte solo local elimina la
...
O deja que el agente lo instale por ti. El skill [`agent-kill-switch`](skills/agent-kill-switch/SKILL.md)
...
| [Sobrecarga](docs/OVERHEAD.md) | Qué cuesta la instrumentación, medido, con el arnés para reproducirlo |
| [Entitlements](docs/ENTITLEMENTS.md) | Gratis frente a pago, matriz de niveles, CLI de licencias |
| [Aprobaciones y políticas](docs/APPROVALS.md) | Bloqueo previo a la ejecución, puntuación de riesgo, aprobaciones por teléfono |
...
| [NemoClaw / OpenShell](docs/NEMOCLAW.md) | Configuraciones aisladas (sandbox) de NVIDIA NemoClaw |
...
Cada número de abajo proviene de una máquina real, en modo solo lectura, sin nada preparado de antemano.
...
pico de costo de 4.2x. Debajo, 324 de 667 sesiones recientes con una señal
...
cuenta. 14 agentes aquí, 3 sesiones trabajando, 13 en silencio.
...
a Bash y cada llamada al modelo tiene su propia barra en la línea de tiempo, así que el comando que corrió
...
ejecuciones con muy poca actividad para evaluar quedan fuera de la calificación en vez de
contarse como victorias. Cada ejecución problemática enlaza a su traza.
...
**La detección funciona sin que tengas que configurar nada.**
Los detectores integrados están activos desde la instalación: agente se quedó callado, el feed de
...
de seguridad. Tus propias reglas son opcionales, encima de todo esto.

![Alerts: built-in detectors plus optional custom rules](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/alerts.png)

**Retener una llamada riesgosa es opcional, y viene desactivado.**
Los borrados recursivos, los force push, sudo, secretos, instalaciones de paquetes y llamadas salientes
tienen cada uno una regla que puedes activar. Hasta que lo hagas, ClawMetry observa y
no cambia nada. Una vez que una regla está activa, las llamadas que coinciden esperan aquí (o en tu teléfono)
para un aprobar o un denegar.
