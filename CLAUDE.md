...
- **Embedded frontend, no build step** — the live UI is served from `clawmetry/static/` (`clawmetry/static/css/dashboard.css`, `clawmetry/static/js/app.js`) + `clawmetry/templates/tabs/*.html`. `dashboard.py` holds exactly one `DASHBOARD_HTML`, and it only `{% include %}`s those files; edit them, not it. (Until 2026-09-08 there were two, and the first — 5,036 dead lines — never rendered; `tests/test_dashboard_html_defined_once.py` stops that returning.) No npm, no webpack.
